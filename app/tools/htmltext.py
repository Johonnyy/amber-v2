...
Stdlib only, deliberately. This repo runs on a handful of dependencies and a
readability library is a large one to add for a single tool; `html.parser` gets most
of the way there, and everything here is pure and offline so it can be tested
exhaustively without a network. If extraction quality turns out to be the thing
limiting `read_url` in practice, `trafilatura` is the upgrade — but that should be a
decision made against real pages, not assumed up front.
...
class _Extractor(HTMLParser):
    ...
    def handle_data(self, data: str) -> None:
        ...
        # as spaces. Only block boundaries make a line break, so they're collapsed
        # here and emitted there. (This does flatten <pre>, which is the right
        # trade for readable prose and wrong for code listings.)
